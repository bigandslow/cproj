...
from cproj import GitWorktree, CprojError
...
detached"""
    ...
    def test_git_status_operations(self, mock_run, temp_repo):
        ...
        result = subprocess.run(expected_cmd, cwd=temp_repo, capture_output=True, text=True, check=False)
...
"""
    ...
    def test_git_worktree_path_validation(self, temp_repo):
        ...
        for path in valid_paths:
            ...
            try:
                relative = path.resolve().relative_to(temp_repo.resolve())
                is_inside_repo = True
            except ValueError:
